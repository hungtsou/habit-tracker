...
describe('<handlerFn>', () => {
  ...
  it('should return 200 with data when request succeeds', async () => {
    vi.mocked(db.<queryFn>).mockResolvedValue(<mockResult>);
    const req = mockReq();
    const res = mockRes();
  ...
  it('should call next with AppError when db query fails', async () => {
    vi.mocked(db.<queryFn>).mockRejectedValue(new Error('db error'));
...
**If a new table is needed**, create `src/db/migrations/<NNN>_create_<table>.sql`:
...
// Middleware order: authenticate → validate(schema) → controller
